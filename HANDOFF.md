...
- **v2** (`wits-quest-map-v2.html`, uses `campus_v2.geojson` inline): user's actual complaint was diagnosed as too many colors / no cohesion, not the cartoon-extrusion concept itself. Fix applied:
  ...
  - **1 accent color** (`#1F4E8C`, Wits blue) reserved *only* for **named buildings** — this conveniently makes likely quest/landmark sites visually distinct from filler scenery for free.
...
| `campus_v2.geojson` *(only exists in `/home/claude/wits-map/`, not yet copied to outputs — copy if needed)* | v2 enriched geojson, current palette |
