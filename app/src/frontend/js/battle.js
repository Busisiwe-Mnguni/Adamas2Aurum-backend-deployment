...
function onCardClick(event) {
    ...
    if (pendingTargetType) {
        const clickedSlot = parseInt(cardElement.getAttribute("data-slot"), 10);

        if (pendingTargetType === "OPPONENT" && isOpponentCard) {
            sendAttack(pendingAttackerSlot, clickedSlot, pendingAction);
            clearSelectionState();
            return;
        }
...
function clearSelectionState() {
    cancelTargeting();
    document
        .querySelectorAll(".battle-card.selected")
            ...
            try {
                ...
                if (data.type === "match-results") {
                    ...
                    elResultHeading.textContent =
                        data.winner === data.user_id ? "Victory" : "Defeat";
                    elResultDamageSuffered.textContent = 0;
                    elResultDamageInflicted.textContent = 0;
                }
                if (data.state !== undefined && data.user_id !== undefined)
                    refreshBattleView(battle_id, data.user_id, data.state);
                if (data.type === "turn_result") {
                    let player_cards, opponent_cards;
                    if (data.state.player1_id === data.user_id) {
                        player_cards = data.state.cards.player1;
                        opponent_cards = data.state.cards.player2;
                    } else {
                        player_cards = data.state.cards.player2;
                        opponent_cards = data.state.cards.player1;
                    }
                    refreshBattleLogs(
                        data.player_result,
                        player_cards,
...
async function buildDeck(deck) {
    try {
        const res = await fetch(`${API_BASE}/api/battles/build-battle-deck`, {
            credentials: "include",
            method: "POST",
...
function refreshDeck() {
    ...
    for (const deckCard of battleDeck) {
        if (deckCard.card == null) {
            ...
            if (deckCard.cardElement != null) deckCard.cardElement.remove();
            continue;
        }
        if (deckCard.cardElement == null) {
...
function addToDeck(card) {
    for (const deckCard of battleDeck) {
        if (deckCard.card != null && card.card_id == deckCard.card.card_id)
            return;
    }
    for (const deckCard of battleDeck) {
        if (deckCard.card == null) {
...
async function loadSelectionEvents() {
    ...
    try {
        ...
        if (data.length == 0) elCardCollectionEmpty.classList.remove("hidden");
...
async function checkAccess() {
    try {
        ...
        const res2 = await fetch(`${API_BASE}/api/battles/find-battle`, {
            credentials: "include",
        });
        if (res2.ok) {
