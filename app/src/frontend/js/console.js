...
async function loadQuestions(eventId) {
    ...
    try {
        const res = await fetch(`${API_BASE}/api/events/${eventId}/questions`, {
            credentials: "include",
        });
...
function buildQuestionItem(q) {
    ...
    li.querySelector('[data-q-action="edit"]').addEventListener("click", () =>
        openQuestionEdit(q),
    );
    li.querySelector('[data-q-action="delete"]').addEventListener("click", () =>
...
function syncOptionsForType(type, existing) {
    if (type === "MULTIPLE_CHOICE") {
        ...
        const opts =
            Array.isArray(existing) && existing.length ? existing : ["", ""];
