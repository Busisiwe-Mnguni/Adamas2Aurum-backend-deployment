...
function is_valid_attacker(cards, slot_position) {
    ...
    if (card.health <= 0) return { ok: false, reason: "That card is defeated" };
    return { ok: true, card };
}
...
function resolve_action(attacker, target, action) {
    switch (action) {
        ...
        case "DODGE":
            return apply_buff(attacker, target, "stat_location", 10, 1);
        case "BUFF":
            return apply_buff(
                attacker,
                target,
        ...
        case "REVIVE":
            return apply_revive(target, Math.round(target.stat_legacy / 2), 3);
        default:
...
function check_battle_over(state) {
    if (!state.cards.player1.some((c) => c.health > 0)) return state.player2_id;
    if (!state.cards.player2.some((c) => c.health > 0)) return state.player1_id;
    return -1;
}
...
battleWss.on("connection", (ws, request) => {
    ...
    ws.on("message", async (raw) => {
        ...
        try {
            ...
            if (msg.type === "join_battle") {
                const state = await load_battle_state(pool, msg.battle_id);
                if (!state)
                    return ws.send(
                        JSON.stringify({
                            type: "error",
            ...
            if (msg.type === "attack") {
                ...
                let player_cards, opponent_cards, player_id, opponent_id;
                    ...
                    if (opponent_id === null) {
                        ...
                        if (cpu_result) {
                            await log_turn(
                                ...
                                state.turn_number,
                                null,
                                opponent_cards[cpu_result.attacker_slot],
                                player_cards[cpu_result.target_slot],
                                cpu_result.action,
                                cpu_result.result,
                            );
                        }

                        winner = check_battle_over(state);
                        state.turn_number += 0.5;
                ...
                if (winner !== -1) {
                    set_battle_finished(battle_id, "COMPLETED", winner);
                    persist_final_health(state);
                    clear_battle_state(battle_id);
                    if (state.player1_id !== null) {
                        const tmp_connection = get_player_connection(
                            state.player1_id,
                        );
    ...
    ws.on("error", (ev) => {
        console.log(`[WebSocket Error] ${new Date().toISOString()} - `, ev);
    });
