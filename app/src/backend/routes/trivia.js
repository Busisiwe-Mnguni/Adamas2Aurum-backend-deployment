...
router.get('/event/:eventId', requireAuth, async (req, res) => {
  try {
    ...
    const { lat, lng, accuracy, qr_verified, location_check_id } = req.query
...
router.post('/submit', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const elapsed_fraction = time_limit_ms > 0
      ? Math.min(1, Math.max(0, elapsed_capped / time_limit_ms))
      : 0
    ...
    if (req.body.qr_verified === true && req.body.location_check_id) {
      const [checkRows] = await pool.query(
      ...
      locationStatus = checkRows.length ? 'FALLBACK_QR' : 'FAILED'
    }
    ...
    const locationVerified = locationStatus === 'VERIFIED' || locationStatus === 'FALLBACK_QR'
    ...
    const pointsAwarded = (isCorrect && locationVerified && !timedOutFinal)
      ? Math.max(1, Math.round(baseReward * (1 - elapsed_fraction / 2)))
      : 0
    ...
    try {
      ...
      award = (isCorrect && locationVerified && !timedOutFinal)
        ? await awardCardIfEligible(conn, { user_id, event_id, elapsed_fraction })
      ...
      await conn.query(
        `INSERT INTO trivia_attempts
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [user_id, event_id, question_id, locCheck.insertId, isCorrect, elapsed_capped, award.card_id, pointsAwarded]
      )
