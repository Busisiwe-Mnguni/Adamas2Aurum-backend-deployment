...
function requireEventAuthor(req, res, next) {
    ...
    pool.query(
        ...
        (err, rows) => {
            ...
            if (!rows.length) return res.status(403).json({ error: 'Forbidden — event author role required' })
            next()
        }
    )
}
...
// ============================================================
router.get('/:id/qr-token', requireAuth, requireEventAuthor, async (req, res) => {
...
router.post('/:id/verify-qr', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const [locCheck] = await pool.query(
            ...
            [req.session?.user?.user_id || req.user?.user_id, eventId]
        )
