...
router.get('/:eventId/pool', requireAuth, requireEventAuthor, async (req, res) => {
	try {
...
router.post('/:eventId/pool', requireAuth, requireEventAuthor, async (req, res) => {
	const { card_id, weight, global_copy_limit } = req.body

	if (!card_id || !Number.isInteger(Number(card_id)) || Number(card_id) < 1) {
		return res.status(400).json({ error: 'card_id is required and must be a positive integer' })
	}
	...
	const limit = global_copy_limit != null && global_copy_limit !== ''
		? parseInt(global_copy_limit, 10)
		: null
	if (limit !== null && limit < 1) {
		return res.status(400).json({ error: 'global_copy_limit must be a positive integer or null' })
	}
	...
	try {
		...
		res.status(201).json({ message: 'Card added to pool', pool_id: result.insertId })
	} catch (err) {
		if (err.code === 'ER_DUP_ENTRY') {
			return res.status(409).json({ error: 'Card is already in this event\'s pool' })
		}
		res.status(500).json({ error: err.message })
	}
})

router.put('/:eventId/pool/:poolId', requireAuth, requireEventAuthor, async (req, res) => {
	...
	if (!sets.length) {
		return res.status(400).json({ error: 'Provide at least weight or global_copy_limit' })
	}
...
router.delete('/:eventId/pool/:poolId', requireAuth, requireEventAuthor, async (req, res) => {
	try {
		const [result] = await pool.query(
