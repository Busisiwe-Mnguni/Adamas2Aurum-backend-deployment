...
var kBroadcastChannel = /* @__PURE__ */ Symbol.for(
...
var nanostoresGlobal =
	globalThis.nanostoresGlobal ||
	(globalThis.nanostoresGlobal = { epoch: 0 })
var drainQueue = () => {
	for (
		lqIndex = 0;
	...
	let $atom = {
		...
		listen(listener) {
			...
			return () => {
				for (
					let i =
						lqIndex +
						QUEUE_ITEMS_PER_LISTENER;
					i < listenerQueue.length;
				) {
					if (listenerQueue[i] === listener) {
						listenerQueue.splice(
							i,
							QUEUE_ITEMS_PER_LISTENER
						)
					} else {
		...
		notify(oldValue, changedKey) {
			...
			let runListenerQueue =
				!listenerQueue.length && !batchSeen
		...
		$store.off = () => {
			...
			setTimeout(() => {
				if ($store.active && !$store.lc) {
					$store.active = false
					for (let destroy of $store.events[
						UNMOUNT
					])
						destroy()
					$store.events[UNMOUNT] = []
...
function parseISODate(value) {
	...
	if (offsetSign) {
		const offset =
			(parseInt(offsetHour, 10) * 60 +
				parseInt(offsetMinute, 10)) *
			(offsetSign === '+' ? -1 : 1)
...
function betterJSONParse(value, options = {}) {
	...
	if (
		Object.entries(PROTO_POLLUTION_PATTERNS).some(
			([key, pattern]) => {
...
function createSessionRefreshManager(opts) {
	...
	const triggerRefetch = (event) => {
		...
		if (event?.event === 'visibilitychange') {
			if (
				now2() - state.lastSessionRequest <
				FOCUS_REFETCH_RATE_LIMIT_SECONDS
			)
				return
			state.lastSessionRequest = now2()
			fetchSession()
	...
	const setupOnlineRefetch = () => {
		state.unsubscribeOnline = getGlobalOnlineManager().subscribe(
			(online) => {
				if (online)
					triggerRefetch({
...
var InternalAPIError = class extends Error {
	constructor(
		...
		body = void 0,
		headers = {},
		statusCode = typeof status === 'number'
			? status
			: statusCodes[status]
	) {
		super(
			body?.message,
			body?.cause ? { cause: body.cause } : void 0
		)
...
function checkHasPath(url) {
	try {
		return (
			(trimTrailingSlashes(new URL(url).pathname) || '/') !==
			'/'
		)
	} catch {
...
function getSessionAtom($fetch, options) {
	...
	const executeSessionFetch = async (signal, queryParams) => {
		...
		try {
			...
			const current2 = session.value
			const stableData =
				current2.data != null &&
				sessionData != null &&
				isJsonEqual(current2.data, sessionData)
					? current2.data
					: sessionData
			session.set({
				data: stableData,
...
var initializePlugins = async (url, options) => {
	...
	for (const plugin of (options == null ? void 0 : options.plugins) ||
		...
		hooks.onError.push(
			(_e = plugin.hooks) == null ? void 0 : _e.onError
		)
		hooks.onRetry.push(
			(_f = plugin.hooks) == null ? void 0 : _f.onRetry
		)
	}
	return {
		url,
...
async function getHeaders(opts) {
	const headers = new Headers(
		mergeHeaders(
			...
			await getAuthHeader(opts)
		)
	)
	if (!headers.has('content-type')) {
		const contentType = detectContentType(
			opts == null ? void 0 : opts.body
		)
		if (contentType) {
...
function getMethod(url, options) {
	...
	if (url.startsWith('@')) {
		...
		if (!methods.includes(pMethod)) {
			return (options == null ? void 0 : options.body)
				? 'POST'
				: 'GET'
		}
...
var betterFetch = async (url, options) => {
	...
	for (const onResponse of hooks.onResponse) {
		if (onResponse) {
			const r = await onResponse(
				__spreadProps(
					__spreadValues({}, responseContext),
					{
						response: (
							(_c =
								options == null
	...
	if (options == null ? void 0 : options.retry) {
		...
		const _retryAttempt =
			(_h = options.retryAttempt) != null ? _h : 0
		if (
...
var getClientConfig = (options, loadEnv) => {
	...
	const { $sessionSignal, session, broadcastSessionUpdate } =
		getSessionAtom($fetch, options)
...
var client_entry_default = authClient
export { client_entry_default as default }
//# sourceMappingURL=auth-client.bundle.mjs.map
